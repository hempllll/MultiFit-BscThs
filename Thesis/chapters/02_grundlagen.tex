...
Die Laufzeit setzt sich aus $k+1$ Aufrufen der Subroutine zusammen: den $k$ Tests der Schleife
und dem Aufruf für den Rückfall.
Die Aufgabenliste wird dabei einmal sortiert, und jeder Aufruf kostet mit einer geeigneten
Datenstruktur für First Fit $O(n \log n)$.
Insgesamt ergibt das $O(k\,n \log n)$, bei fester Suchtiefe $k$ also $O(n \log n)$
\cite[S.~35]{friesen_langston1986}.

...
Neben der Schranke für MULTIFIT+FFD ist sie die einzige Makespan-Garantie dieser Arbeit, welche
allein von $d$ und $k$ abhängt; die Schranke, die Abschnitt~\ref{sub:anwendbarkeit} für den
$\mathrm{OPT}+1$-Algorithmus angibt, hängt dagegen von der Instanz selbst ab.

Für das Konfigurations-LP von \textcite{gilmore_gomory1961} fällt das Ergebnis schwächer aus.
Lemma~\ref{lem:gg-rundung} lässt nach dem Runden $m + d - 1$ Bins zu und trägt
Implikation~\eqref{eq:ffd-akzeptiert} damit nicht für $r = 1$; erst ab $m \ge 2(d-1)$ trägt sie
mit $r = 2$, und eine von $d$ unabhängige Konstante liefert das Lemma nicht.
...
Eine im Mittel bessere Binzahl schlägt also nicht auf den Makespan durch, sondern erst die
Frage, bei welchen Kapazitäten die Binzahl die Schwelle $m$ nicht überschreitet; genau an diese
Entscheidung knüpft Implikation~\eqref{eq:ffd-akzeptiert} die Gütegarantie von MULTIFIT+FFD.
...
und ob MULTIFIT+MFFD eine kleinere Schranke als MULTIFIT+FFD erfüllt, bleibt offen -
Definition~\ref{def:rm} legt $r_m$ allein über FFD fest, sodass die Frage für MFFD erst
über eine eigene Fassung dieser Definition zu stellen wäre.

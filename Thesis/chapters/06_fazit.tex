...
Die Schranke $13/11$ selbst bleibt unbewiesen; Abschnitt~\ref{sec:beweisidee1311} stellt nur
ihre Beweisidee dar.
...
überhaupt nicht auswirkt: Ab $\rho(\Gamma) \ge 2$ erzeugen beide Subroutinen bei jeder
...
$(1+\varepsilon)(1+2^{-k}) \cdot C^{*}_{\max}(\Gamma)$ mit $\varepsilon = 1/(2d-1)$.
Neben der Schranke für MULTIFIT+FFD ist sie die einzige Makespan-Garantie dieser Arbeit.
...
Makespan, mit dem $\varepsilon$-dualen Algorithmus sind es \HmOptimaljansendual.
Gegenüber der Grundlinie gewinnt MULTIFIT mit dem $\mathrm{OPT}+1$-Algorithmus dennoch nicht
durchgängig: \HmJansenBesserFFD{} Instanzen fallen besser aus, \HmJansenSchlechterFFD{}
schlechter.
...
herankommt, sondern welche Kapazitäten sie annimmt.
Genau das besagt Implikation~\eqref{eq:ffd-akzeptiert}.
...
von~\eqref{eq:ffd-akzeptiert} überhaupt besteht: Für FFD ist sie bewiesen, für das
Konfigurations-LP widerlegt, für MFFD unbekannt.
Unbestimmt bleibt ferner das Grenzverhältnis für kleine Maschinenzahlen, denn
Lemma~\ref{lem:untere-schranke} setzt $m \ge 13$ voraus; für $m < 13$ klafft zwischen
...
Über $\varepsilon = 1/(2d-1)$ wird ihre Garantie mit wachsender Anzahl verschiedener
Aufgabengrößen schärfer, die Laufzeit des Verfahrens nach Abschnitt~\ref{sub:jansen-laufzeit}
jedoch doppelt exponentiell in $d$.

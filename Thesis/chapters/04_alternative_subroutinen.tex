...
Hier wird stattdessen durchgehend die in Kapitel~\ref{chap:grundlagen} festgelegte Notation
...
\begin{bemerkung}
Der Beweis nutzt von der oberen Intervallgrenze nichts und gilt daher für jede der
gebräuchlichen Varianten, etwa $c_{\mathrm{high}}^{(0)} = \sum_j p_j/m + p_{\max}$ oder
$c_{\mathrm{high}}^{(0)} = \max(\sum_j p_j/m,\, 2 p_{\max})$.
Er überträgt sich außerdem auf die Variante mit ganzzahligen Kapazitäten:
...
\]
Ab $n/m \ge 4$ ist demnach zu erwarten, dass $\rho(\Gamma) \ge 2$ gilt und beide Subroutinen dieselbe Ausgabe liefern.
Das ist eine Aussage über den Erwartungswert und keine Garantie für die einzelne Instanz - Instanzen mit wenigen sehr großen und vielen sehr kleinen Aufgaben verletzen $\rho(\Gamma) \ge 2$ auch bei großem $n/m$.
Lemma~\ref{lem:kollaps} ist von der Größenverteilung dagegen unabhängig.

Der Austausch der Subroutine kann sich also nur auf Instanzen mit $\rho(\Gamma) < 2$ auswirken.
...
Für die Schranke $13/11$ ist die Klasse ohne Belang, denn der bekannte Extremfall liegt außerhalb:
Die Instanz von \textcite{friesen1984}, auf der MULTIFIT+FFD das Verhältnis $13/11$ annimmt, hat
$\rho = 858/(13 \cdot 40) = 1{,}65 < 2$ (Abschnitt~\ref{sec:friesen}).
Das Lemma grenzt also ein, wo ein Unterschied zwischen den beiden Subroutinen überhaupt möglich
ist; eine schärfere obere Schranke für MULTIFIT+MFFD folgt daraus nicht.
...
Diese Darstellung wird in diesem Kapitel durchgehend verwendet; die Kapazität $C$ ist dabei ganzzahlig.
...
Im Originalalgorithmus wird über alle Teilmengen $S^B \subseteq \mathcal{K}^B$ der Größe $2^d$ iteriert,
da Lenstras Algorithmus eine feste obere Schranke an die Anzahl ganzzahliger Variablen voraussetzt.
Mit Gurobi entfällt diese Einschränkung: Es genügt, ein einziges MILP mit allen großen Konfigurationen
als ganzzahlige Variablen aufzustellen. Nach \textcite{eisenbrand_shmonin2006} (dort zitiert in \cite{jansen_solisoba2010})
setzt die optimale Lösung ohnehin höchstens $2^d$ dieser Variablen auf einen Wert größer null -
Gurobi findet diese aktiven Konfigurationen selbst.
Die Subset-Enumeration wäre in der Praxis auch gar nicht durchführbar: Für $d = 3$ gibt es bereits
$\binom{125}{8} \approx 10^{12}$ Teilmengen, für $d = 5$ etwa $10^{117}$ -
...
Kleine Aufgaben, die nach dem Abrunden nicht abgedeckt sind, werden gesammelt und in einen
einzigen zusätzlichen Bin gepackt.
Dass alle Reste dort hineinpassen, folgt aus dem in \cite{jansen_solisoba2010} geführten
Rundungsargument; die Packung verwendet damit insgesamt höchstens $m^* + 1$ Bins.
...
$\mathrm{J}[\Gamma, \cdot\,]$ nicht monoton fallen; ein Zeuge dafür wird in
...
Passen sie nicht, entsteht ein zusätzlicher Bin und $\mathrm{J}[\Gamma, C] = \mathrm{OPT}[\Gamma, C]+1$.
...
Bezeichnet $C^{*}_{\max}(\Gamma, m')$ den optimalen Makespan von $\Gamma$ auf $m'$ Maschinen -
insbesondere $C^{*}_{\max}(\Gamma) = C^{*}_{\max}(\Gamma, m)$ -, so trägt folgende Kette:
...
Liegt $C^{*}_{\max}(\Gamma, m-1)$ im Startintervall, so ist die im Grenzwert $k \to \infty$
resultierende Güte damit durch $C^{*}_{\max}(\Gamma, m-1)/C^{*}_{\max}(\Gamma, m)$ beschränkt;
bei endlichem $k$ kommt der Term $2^{-k}$ hinzu.
...
Für $k \to \infty$ konvergiert die Güte gegen $(1+\varepsilon)\cdot C^{*}_{\max}(\Gamma)$.
...
Der Algorithmus aus Abschnitt~\ref{sub:anwendbarkeit} gibt höchstens $\mathrm{OPT}[\Gamma, C]+1$
Bins aus; der Extrabin entsteht in Schritt~3, weil die verbliebenen kleinen Aufgaben dort
in einen zusätzlichen Bin der Kapazität~$C$ gepackt werden \cite{jansen_solisoba2010}.
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item Weise jedem der $m^*$ Bins seine große Konfiguration und die abgerundeten kleinen Aufgaben zu.
  \item Sammle die verbliebenen kleinen Aufgaben $L$ (nicht abgedeckt durch die gerundeten $\bar{x}$-Werte);
        nach Lemma~3.1 in \cite{jansen_solisoba2010} gilt $|L| \le 2d-1$
        (das Lemma gilt hier unverändert, da die Schranke allein aus dem Rundungsargument der Schritte~1 und~2 folgt, die gegenüber dem Original unverändert sind).
...
\label{lem:jansen-dual}
  Sei $\Gamma$ eine Instanz mit $d$~verschiedenen Größen, $C \ge p_{\max}$ eine Kapazität
  und $\varepsilon = 1/(2d-1)$; $d$ sei konstant, sodass die Laufzeit nach
  Abschnitt~\ref{sub:jansen-laufzeit} polynomiell ist.
...
$m^*$ oder $m^*+1$: Der zusätzliche Bin aus Schritt~3 wird genau dann geöffnet, wenn nach dem
Abrunden der $x^+$-Werte kleine Aufgaben übrig bleiben.
...
Dieselbe Modellierung liegt schon dem ältesten Zugang zum Bin Packing zugrunde, dem linearen
...
Weil die Kapazitäten ganzzahlig sein müssen - $\mathcal{K}(\Gamma, C)$ ist nur für ganzzahliges
$C$ aufzählbar - wird die Variante der Binärsuche mit ganzzahligen Kapazitäten verwendet.
...
Eine Gütegarantie für den Makespan überträgt sich daraus nicht.
Die Schranke von \textcite{cao1995} für MULTIFIT+FFD beruht auf
...
Lemma~\ref{lem:gg-rundung} liefert für $C \ge C^{*}_{\max}(\Gamma)$ aber nur
$\mathrm{GG}[\Gamma, C] \le m + d - 1$ und lässt damit gerade die $d-1$ Bins zu, die das
Akzeptanzkriterium nicht mehr zulässt.

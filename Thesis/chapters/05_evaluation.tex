...
Dieser Datensatz geht auf Lawrinenko zurück und wurde von Mrad und Souayah für einen
Arc-Flow-Ansatz auf $P\,\|\,C_{\max}$ eingesetzt \cite{mrad2018}.
...
Berndt, Deppert, Jansen und Rohwedder entwickelten diesen Datensatz als
Wettbewerbs-Benchmark für $P\,\|\,C_{\max}$ \cite{berndt2022};
er wird u.\,a.\ in \textcite{akram2025} eingesetzt.
...
Für das Konfigurations-LP gibt es überhaupt keine.
Der Grund steht in Abschnitt~\ref{sub:gg-multifit}: Die Schlussweise hinter der Schranke
$13/11$ verlangt eine Konstante $r$, für die jede Kapazität $C \ge r \cdot C^{*}_{\max}(\Gamma)$
mit $m$ Bins auskommt (Implikation~\eqref{eq:ffd-akzeptiert}); Lemma~\ref{lem:gg-rundung} lässt
dort $m + d - 1$ zu.

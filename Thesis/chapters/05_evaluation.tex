...
Die vier High-Multiplicity-Subroutinen setzen ein kleines, konstantes $d$ voraus - beim
$\mathrm{OPT}+1$-Algorithmus und seiner $\varepsilon$-dualen Variante wegen der Laufzeit
(Abschnitt~\ref{sub:anwendbarkeit}), beim Konfigurations-LP, weil die Spaltenaufzählung mit $d$
wächst und der additive Term aus Lemma~\ref{lem:gg-rundung} gerade $d-1$ beträgt.
Die fünf Benchmark-Datensätze bieten das nicht: Dort
liegt $d$ im Median zwischen \HmDBenchMedianMin{} und \HmDBenchMedianMax{} und erreicht im
Maximum \HmDBenchMax.
Beide Reihen benutzen deshalb verschiedene Datensätze.
Verschieden ist außerdem die Suche: Abschnitt~\ref{sub:hm-aufbau} testet nur ganzzahlige
Kapazitäten, Abschnitt~\ref{sec:greedy} dagegen reellwertige, sodass auch die Laufzeiten nicht
unmittelbar vergleichbar sind.
Jede Reihe beginnt mit
...
In keiner davon unterscheiden sich die gemessenen Makespans, wie es Lemma~\ref{lem:kollaps}
verlangt.

Abbildung~\ref{fig:kollaps} stellt das Kriterium dem Verhältnis $n/m$ gegenüber, auf das sich
der Erwartungswert von $\rho(\Gamma)$ in Abschnitt~\ref{sub:kollaps} zurückführen lässt.
Auch oberhalb von $n/m = 12$ tritt keine Abweichung auf (\EvalNMZwoelf{} Instanzen); dieser
Wert ist jedoch an den Daten abgelesen und nicht hergeleitet.
Das $n/m$-Kriterium trennt zudem weniger scharf: Die Abweichung mit dem größten $n/m$ liegt bei
$n/m = \EvalNMMaxAbw$ auf und damit deutlich oberhalb von $n/m = 4$, ab dem der Erwartungswert
von $\rho(\Gamma)$ nach Abschnitt~\ref{sub:kollaps} den Wert $2$ erreicht - woraus für die
einzelne Instanz nichts folgt.
...
Die Schwelle aus Lemma~\ref{lem:kollaps} ist auf diesen Datensätzen also nicht nur hinreichend,
...
Der Mehraufwand ist damit messbar, bleibt auf diesen Datensätzen aber in derselben
Größenordnung.

\begin{table}[htbp]
\centering
\input{tables/tab_laufzeit}
...
\centering
\small
\input{tables/tab_k_sensitivitaet}
...
Bei FFD ist das ausgeschlossen: Hat die von FFD bei Kapazität $C$ erzeugte Packung die größte
Binlast $L$, so trifft FFD für jede Kapazität zwischen $L$ und $C$ dieselben
Platzierungsentscheidungen und liefert dieselbe Packung; die gespeicherte Binlast kann mit
wachsendem $k$ also nicht steigen.
MFFD teilt die Aufgaben dagegen relativ zu $C$ in Klassen ein und kann bei kleinerem $C$
schlechter packen.
Über alle \EvalInstanzen{} Instanzen und alle Übergänge $k \to k+1$ zählt die Messung
entsprechend \EvalKNichtMonotonFFD{} solche Fälle bei MULTIFIT+FFD und \EvalKNichtMonotonMFFD{} bei
...
Ob MULTIFIT+MFFD eine kleinere Schranke als MULTIFIT+FFD erfüllt, ist offen; $r_m$ ist nach
Definition~\ref{def:rm} allein über FFD festgelegt.
...
\HmInstanzen{} Instanzen aus einem im Entwurf vollständig balancierten Gitter:
...
dort angenommen.
Auf allen \HmDegeneriert{} erreichen \HmDegAlleOptimal{} fünf Subroutinen den optimalen
Makespan; sie trennen die Verfahren nicht und bleiben in den Gütevergleichen unberücksichtigt.
Tabelle~\ref{tab:hm-ratio-opt} beruht deshalb auf den übrigen Instanzen, während der
Makespan-Vergleich in Tabelle~\ref{tab:hm-paare} alle \HmInstanzen{} umfasst.
...
Unterschied.
$\mathrm{J}$ wird mit der inneren Binärsuche gemessen, also in der Suchstruktur der
publizierten Fassung; im Verteilen der Reste weicht die Implementierung nach
Abschnitt~\ref{sub:jansen-implementierung} von ihr ab.
...
Theorem~\ref{thm:guete} überträgt sich auf diese Variante: Bei ganzzahligen Größen ist jede
Binlast ganzzahlig, also trifft FFD bei jedem $C' \in [C, C+1)$ dieselben Entscheidungen wie
bei $C$. Wird $C$ verworfen, so gilt nach~\eqref{eq:ffd-akzeptiert}
$C' \le r_m \cdot C^{*}_{\max}(\Gamma)$ für alle diese $C'$, im Supremum also
$C+1 \le r_m \cdot C^{*}_{\max}(\Gamma)$ - die Invariante überlebt das Setzen von
$c_{\mathrm{low}} := C+1$. Da sich die Intervallbreite in beiden Zweigen mindestens halbiert,
trägt auch der Term $2^{-k}$.
Für Korollar~\ref{kor:multifit-jansen-dual} ist das nicht geführt.
...
Die Spalte zu MULTIFIT+FFD steht der bewiesenen Schranke $13/11 + 2^{-k} \approx \HmSchrankeK$
nach \textcite{cao1995} gegenüber; sie gilt nach Abschnitt~\ref{sub:hm-aufbau} auch für die
hier gemessene ganzzahlige Suche.
Die Spalte zu MULTIFIT+$\mathrm{J}_\varepsilon$ steht dem Wert $(1+\varepsilon)(1+2^{-k})$ aus
Korollar~\ref{kor:multifit-jansen-dual} gegenüber, wobei $\varepsilon = 1/(2d-1)$ hier zwischen
\HmEpsTheorieMin{} und \HmEpsTheorieMax{} liegt.
Dieser Wert ist nur eine Vergleichslinie: Gemessen wird die Fassung ohne innere Binärsuche,
für die das Argument am Ende von Abschnitt~\ref{sub:eps_dual} trägt und nicht
Korollar~\ref{kor:multifit-jansen-dual} selbst.
Für MULTIFIT+$\mathrm{J}$ beziffert Abschnitt~\ref{sub:anwendbarkeit} das Verhältnis zum
Optimum je Instanz durch $C^{*}_{\max}(\Gamma, m-1)/C^{*}_{\max}(\Gamma, m)$; über alle
Instanzen genommen ergibt das nur den Wert~$2$ und damit keine mit $13/11$ vergleichbare
Aussage.
Für das Konfigurations-LP zeigt Abschnitt~\ref{sub:gg-multifit}, dass
Implikation~\eqref{eq:ffd-akzeptiert} mit $r = 2$ trägt, sobald $m \ge 2(d-1)$ gilt - auf dem
HM-Gitter ist das für \HmGgRZweiAnteil{} der Instanzen der Fall.
Eine Schranke gewinnt man daraus nicht, denn das Startintervall hält den Faktor~$2$ ohnehin
ein; ob eine von $d$ unabhängige Konstante $r < 2$ existiert, ist offen.
...
Verglichen werden kann jedoch nur, wo beide Verfahren dieselbe Kapazität getestet haben; die
Binärsuche läuft je Subroutine verschieden, sodass die Grundmenge kleiner ausfällt als die
Spalte \emph{Aufrufe} in Tabelle~\ref{tab:hm-akzeptanz}.

Für den $\mathrm{OPT}+1$-Algorithmus gilt in allen \HmJansenVergleiche{} so verglichenen Aufrufen
$\mathrm{J}[\Gamma,C] - \mathrm{OPT}[\Gamma,C] \in \{0,1\}$; der Wert 1 tritt in
\HmJansenPlusEins{} Fällen auf (\HmJansenUeberOpt), sonst ist die Packung optimal.
Bewiesen ist diese Schranke für die publizierte Fassung; die Implementierung weicht nach
Abschnitt~\ref{sub:jansen-implementierung} im Verteilen der Reste von ihr ab und hält sie
in jedem gemessenen Aufruf ebenfalls ein.

Für das Konfigurations-LP erlaubt Lemma~\ref{lem:gg-rundung} den additiven Term $d-1$, hier also
bis zu \HmDMinusEins.
Gemessen wird er nicht annähernd erreicht: Über \HmGGVergleiche{} vergleichbare Aufrufe ist
...
Schritt~3 aus Abschnitt~\ref{sub:gg-rundung} packt den Restbedarf nicht mit einem Bin je
...
Ausgeschöpft wird das nicht: Über die \HmEpsAufrufe{} Aufrufe, bei denen das Programm lösbar
war und eine Packung entstand, überschreitet nur in \HmEpsPositiv{} Fällen überhaupt ein Bin
die Kapazität $C$.
Die größte gemessene Überschreitung beträgt \HmEpsMax{} und entspricht einer Last von höchstens
$\HmLastFaktorMax \cdot C$.
...
Jede dieser \HmZuUnrechtJansen{} Ablehnungen tritt gemessen bei $\mathrm{J}[\Gamma,C] = m+1$
auf - dem Fall, der nach Abschnitt~\ref{sub:anwendbarkeit} zweideutig ist und deshalb
konservativ als Ablehnung behandelt wird.
...
Für die Richtung, in der der $\mathrm{OPT}+1$-Algorithmus verliert, lässt sich das nachprüfen:
Er verwirft auf \HmJansenFehlablehnungInstanzen{} Instanzen mindestens eine zulässige
Kapazität, und alle \HmJansenVerlusteMitFehlablehnung{} Instanzen, auf denen MULTIFIT mit ihm
den größeren Makespan liefert, gehören dazu; \HmJansenVerlusteOhne{} liegen außerhalb.
...
\centering
\small
\input{tables/tab_hm_skalierung}
